<h1 align="center">
  <img src="https://raw.githubusercontent.com/ssssmy/HermesCCNotify/main/assets/icon.svg" width="48" height="48" alt="HermesCCNotify" valign="middle">
  HermesCCNotify
</h1>
...
**HermesCCNotify** 解决的就是这个。它挂载到 Claude Code 的原生事件系统上，任务完成的瞬间推送通知——到你的 Mac、手机、或团队 Slack。
...
Claude Code（v2.x+）内置了 **hooks** 系统——在特定生命周期事件触发时执行 shell 命令。HermesCCNotify 在 `Stop` 事件上注册钩子，Claude 每完成一轮回复就会触发。
...
桥接脚本的运行日志在 `~/.hermesccnotify/bridge.log`：

```bash
# 实时查看
tail -f ~/.hermesccnotify/bridge.log

# 查看最近活动
tail -20 ~/.hermesccnotify/bridge.log
```

### 事件文件

诊断用事件暂存在 `~/.hermesccnotify/events/`（仅当日志开启时）。文件命名格式：`<unix时间戳>-<session_id>.json`。Hermes webhook 集成后不再依赖此目录。

## Hermes 集成

HermesCCNotify 与 **Hermes Agent** 深度集成——通过内置 Webhook 系统实现毫秒级事件驱动投递，彻底告别轮询。
...
- 查看 `~/.hermesccnotify/bridge.log` 里的连接错误。
